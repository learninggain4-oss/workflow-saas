      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-5 custom-scrollbar">
        ...
        <div className={`border rounded-2xl p-4 shadow-sm ${subCard}`}>
          <div className="mb-3 flex items-center justify-between">
            <h2 className="font-semibold text-[10px] uppercase tracking-[0.2em] text-slate-500">Quick access</h2>
          </div>
        ...
        <div>
          ...
          <div className="flex gap-2 px-1">
            <input value={newBoardName} onChange={(e) => setNewBoardName(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && createBoard()} placeholder="New project..." className={`border p-2.5 rounded-xl text-sm flex-1 min-w-0 ${inputCls}`} />
            <button onClick={createBoard} className={`px-3 rounded-xl text-sm font-bold flex items-center justify-center ${primaryBtn}`}>+</button>
          </div>
        </div>
