          ...
          <div key={group.title} className={`flex flex-wrap gap-2 rounded-2xl border p-2 shadow-sm ${bgCard}`}>
            ...
            {group.items.map((m) => (
              <button
                ...
                className={`rounded-xl px-3 py-1.5 text-sm font-semibold capitalize transition-all duration-200 ${viewMode === m ? 'bg-indigo-600 text-white shadow-md shadow-indigo-500/20' : 'text-gray-500 hover:text-gray-900 dark:hover:text-gray-200'}`}
              >
                {m}
              </button>
            ))}
