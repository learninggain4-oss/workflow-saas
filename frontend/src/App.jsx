        ...
        {viewMode==="board"?(
          ...
          <div className={`rounded-xl border p-5 shadow-sm ${bgCard}`}>
            ...
            <div className="grid grid-cols-7 gap-px bg-gray-200 dark:bg-gray-700 border rounded-xl overflow-hidden">
              {["Sun","Mon","Tue","Wed","Thu","Fri","Sat"].map(d=><div key={d} className={`p-2.5 text-xs font-bold text-center uppercase ${subCard}`}>{d}</div>)}
              {Array.from({length:firstDay}).map((_,i)=><div key={`empty-${i}`} className={`min-h-[100px] ${bgCard}`}></div>)}
              {Array.from({length:daysInMonth}).map((_,idx)=>{
                ...
                return (
                  <div key={day} className={`min-h-[100px] p-2 overflow-hidden ${bgCard} ${isToday?'ring-2 ring-black dark:ring-white ring-inset':''}`}>
                    <div className={`text-sm font-bold w-6 h-6 flex items-center justify-center rounded-full ${isToday?'bg-black text-white dark:bg-white dark:text-black':''}`}>{day}</div>
                    <div className="mt-1.5 space-y-1">
                      {dayTasks.slice(0,3).map(t=>(
                        <div key={t.id} onClick={()=>openEditModal(t)} className={`text-xs px-1.5 py-0.5 rounded truncate cursor-pointer font-bold border ${t.priority==='high'?'bg-red-100 text-red-700 border-red-200':'bg-blue-100 text-blue-700 border-blue-200'}`}>{t.title}</div>
                      ))}
                      {dayTasks.length>3 && <div className="text-xs text-gray-400 font-bold">+{dayTasks.length-3} more</div>}
                    </div>
                  </div>
                )
              })}
