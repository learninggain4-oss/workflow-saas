...
export default function App(){
  ...
  const handleRegister=async()=>{
    try{ await axios.post(`${API_URL}/api/register`,{email,password,name}); alert("Registered! Now login"); setIsRegister(false)}
  ...
  const deleteBoard=async()=>{ if(!selectedBoard) return; if(!confirm("Delete board and all tasks?")) return; await axios.delete(`${API_URL}/api/boards/${selectedBoard}`,authHeader); setSelectedBoard(null); await fetchBoards() }
  const inviteUser=async()=>{
    if(!inviteEmail.trim()||!selectedBoard) return
    try{ await axios.post(`${API_URL}/api/boards/${selectedBoard}/invite`,{email:inviteEmail},authHeader); alert("Invited!"); setInviteEmail(""); fetchBoardMembers() }
    catch(e){ alert(e.response?.data?.detail||"Invite failed") }
  }
  const addComment=async()=>{
    if(!newComment.trim()||!editing) return
    try{ await axios.post(`${API_URL}/api/tasks/${editing.id}/comments`,{text:newComment},authHeader); setNewComment(""); fetchComments(editing.id) }
  ...
  const handleFileUpload=async(e)=>{
    ...
    }catch{ alert("Upload failed - add Cloudinary keys") }
    setUploading(false)
  }
  ...
  const toggleLabel=(labelName)=>{
    ...
    const next=cur.includes(labelName)? cur.filter(l=>l!==labelName) : [...cur, labelName]
  ...
  const inputCls = darkMode? "bg-[#2a2e38] border-gray-600 text-white" : "bg-white border-gray-300"
    ...
    <div className={`min-h-screen flex ${bgMain}`}>
      <div className={`w- min-w- border-r p-5 flex flex-col h-screen sticky top-0 overflow-y-auto ${bgSide}`}>
        <div className="flex justify-between items-center mb-6">
          ...
          <button onClick={()=>setDarkMode(!darkMode)} className="border px-3 py-1.5 rounded-lg text-sm bg-black text-white dark:bg-white dark:text-black">{darkMode?"☀️":"🌙"}</button>
        </div>
        <h2 className="font-bold text- uppercase text-gray-500 mb-3">Your Boards</h2>
        <div className="space-y-2 mb-4 max-h- overflow-auto">
          {boards.map(b=>(
            <button key={b.id} onClick={()=>setSelectedBoard(b.id)} className={`w-full text-left p-2.5 rounded-lg text-sm border truncate ${selectedBoard===b.id?'bg-black text-white border-black dark:bg-white dark:text-black': bgCard}`}>📋 {b.name}</button>
          ))}
        </div>
        <div className="flex gap-2 mb-6">
          <input value={newBoardName} onChange={e=>setNewBoardName(e.target.value)} placeholder="New board" className={`border p-2 rounded-lg text-sm flex-1 ${inputCls}`}/>
        ...
        <div className="border-t pt-4 mb-4">
          ...
          <button disabled={!selectedBoard} onClick={inviteUser} className="bg-blue-600 disabled:bg-gray-600 text-white w-full p-2.5 rounded-lg text-sm">Invite + Email</button>
        </div>
        <div className="border-t pt-4">
          <h3 className="font-bold text- uppercase mb-2">Filter 🏷️</h3>
          <div className="flex flex-wrap gap-1.5">
            <button onClick={()=>setFilterLabel("all")} className={`text- px-2.5 py-1 rounded-full border ${filterLabel==="all"?"bg-black text-white":""}`}>All</button>
            {AVAILABLE_LABELS.map(l=>(
              <button key={l.name} onClick={()=>setFilterLabel(l.name)} className={`text- px-2.5 py-1 rounded-full border ${filterLabel===l.name?'bg-black text-white': l.cls}`}>{l.name}</button>
            ))}
          </div>
        </div>
        <button onClick={()=>{localStorage.clear(); setToken(""); setSelectedBoard(null)}} className={`mt-auto text-xs border p-2 rounded-lg ${bgCard}`}>Logout</button>
      </div>
      ...
      <div className="flex-1 p-6 lg:p-8 overflow-auto">
        <div className="flex justify-between items-center mb-6 flex-wrap gap-3">
          ...
          <div className="flex gap-2 items-center flex-wrap">
            ...
            <div className="relative">
              <button onClick={()=>setShowNotif(!showNotif)} className={`relative border px-4 py-2.5 rounded-lg text-sm ${bgCard}`}>🔔 {unread>0 && <span className="absolute -top-2 -right-2 bg-red-500 text-white text- w-5 h-5 flex items-center justify-center rounded-full">{unread}</span>}</button>
              {showNotif && (
                <div className={`absolute right-0 top-12 w- border rounded-xl shadow-xl z-50 max-h- overflow-hidden flex flex-col ${bgCard}`}>
                  <div className={`p-3 border-b flex justify-between ${subCard}`}><span className="font-bold text-sm">Notifications</span><button onClick={markAllRead} className="text- text-blue-500">Mark all read</button></div>
                  <div className="overflow-auto flex-1">
                    ...
                    {notifications.map(n=>(
                      <div key={n.id} className={`p-3 border-b flex gap-2 ${!n.is_read?'bg-blue-50 dark:bg-[#252a33]':''}`}>
                        <div className="flex-1"><p className="text-">{n.message}</p><p className="text- text-gray-400">{n.created_at} • 📧 email sent</p></div>
                        <button onClick={()=>markRead(n.id)} className="text- bg-black text-white px-2 py-1 rounded h-fit">Read</button>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3 mb-6">
          <div className={`rounded-xl border p-4 ${statCard}`}><p className="text- uppercase text-gray-500 font-bold">Total</p><p className="text-2xl font-bold mt-1">{analytics.total}</p><div className="mt-2 w-full bg-gray-200 h-1.5 rounded-full"><div className="bg-black h-1.5 rounded-full" style={{width:`${analytics.progress}%`}}></div></div></div>
          <div className={`rounded-xl border p-4 ${statCard}`}><p className="text- uppercase text-gray-500 font-bold">Done</p><p className="text-2xl font-bold mt-1 text-green-600">{analytics.done}</p><p className="text- text-gray-400">{analytics.progress}% done</p></div>
          ...
          <div className={`rounded-xl border p-4 ${statCard}`}><p className="text- uppercase text-gray-500 font-bold">My Tasks</p><p className="text-2xl font-bold mt-1">{analytics.my}</p></div>
          <div className={`rounded-xl border p-4 ${statCard} col-span-2 lg:col-span-1`}><p className="text- uppercase text-gray-500 font-bold">Labels</p><div className="flex gap-1 flex-wrap mt-2">{Object.entries(analytics.labelCount).map(([k,v])=><span key={k} className={`text- px-2 py-0.5 rounded-full border ${getLabelCls(k)}`}>{k}:{v}</span>)} {Object.keys(analytics.labelCount).length===0 && <span className="text- text-gray-400">No labels</span>}</div></div>
        </div>
                    ...
                    {filtered.filter(t=>t.status===s).map((t,i)=>(
                      <Draggable key={t.id} draggableId={String(t.id)} index={i}>{(pr)=>(
                        <div ref={pr.innerRef} {...pr.draggableProps} {...pr.dragHandleProps} onClick={()=>openEditModal(t)} className={`p-3 rounded-lg mb-3 border cursor-pointer ${bgTask}`}>
                          ...
                          <div className="flex justify-between items-center mt-2"><span className={`text- px-2 py-1 rounded-full font-bold ${t.priority==='high'?'bg-red-100 text-red-600':'bg-green-100 text-green-700'}`}>{t.priority}</span>{t.due_date && <span className="text- text-gray-500">📅 {t.due_date}</span>}</div>
                          {t.assigned_to && <div className="mt-1 text- text-gray-500">👤 {t.assigned_to_name||t.assigned_to.split('@')[0]}</div>}
                          {t.attachment_url && <div className="text- text-blue-500 mt-1">📎 file</div>}
                        </div>
                      )}</Draggable>
                    ))}{p.placeholder}
                  </div>
          ...
          <div className={`rounded-xl border p-5 ${bgCard}`}>
            ...
            <div className="grid grid-cols-7 gap-px bg-gray-200 border rounded-lg overflow-hidden">
              ...
              {Array.from({length:daysInMonth}).map((_,idx)=>{
                const day=idx+1; const dateStr=formatDate(new Date(y,m,day)); const dayTasks=tasksByDate(dateStr)
                const isToday=dateStr===formatDate(new Date())
                return (
                  <div key={day} className={`h- p-2 overflow-hidden ${bgCard} ${isToday?'ring-2 ring-black ring-inset':''}`}>
                    <div className={`text- font-bold w-6 h-6 flex items-center justify-center rounded-full ${isToday?'bg-black text-white':''}`}>{day}</div>
                    <div className="mt-1 space-y-1">{dayTasks.slice(0,3).map(t=><div key={t.id} onClick={()=>openEditModal(t)} className="text- px-1.5 py-0.5 rounded truncate cursor-pointer bg-blue-100 text-blue-700">{t.title}</div>)} {dayTasks.length>3 && <div className="text- text-gray-400">+{dayTasks.length-3}</div>}</div>
                  </div>
                )
              })}
            </div>
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className={`rounded-xl p-6 w- max-w-full max-h- overflow-y-auto ${bgCard}`}>
            ...
            <div className={`border rounded-lg p-3 mb-3 ${subCard}`}>
              <label className="text- font-bold uppercase">Labels 🏷️</label>
              <div className="flex flex-wrap gap-2 mt-2">
                {AVAILABLE_LABELS.map(l=>{
                  const active=(editing.labels||"").split(",").includes(l.name)
                  return <button key={l.name} onClick={()=>toggleLabel(l.name)} className={`text- px-3 py-1.5 rounded-full border font-bold ${active?'bg-black text-white border-black': l.cls}`}>{active?'✓ ':''}{l.name}</button>
                })}
              </div>
            </div>
            <div className={`border rounded-lg p-3 mb-3 ${subCard}`}>
            ...
            <div className={`border rounded-lg p-3 mb-4 ${subCard}`}>
              <label className="text- font-bold uppercase">File 📎</label>
              ...
              {editing.attachment_url && <div className="mt-2 text-xs break-all"><a href={editing.attachment_url} target="_blank" rel="noreferrer" className="text-blue-500">{editing.attachment_url.slice(0,60)}...</a><button onClick={()=>setEditing({...editing,attachment_url:""})} className="text- text-red-500 ml-2">Remove</button></div>}
            </div>
            ...
            <div className="mt-6 border-t pt-4">
              <h3 className="font-bold text-sm mb-3">Comments 💬 + Email</h3>
