...
function App(){
  ...
  useEffect(()=>{
    ...
    wsRef.current = ws
    ws.onopen = ()=> console.log("WS connected board", selectedBoard)
    ws.onmessage = (e)=>{
      try{
  ...
  // Fallback polling every 5 sec if WS fails
  useEffect(()=>{
    if(!selectedBoard ||!token) return
  ...
  const addTask=async()=>{
    if(!title.trim() ||!selectedBoard) return alert("Select a board first")
  ...
  const addComment = async () => {
    ...
    }catch(e){ alert("Comment failed: "+(e.response?.data?.detail||e.message)) }
  }
  ...
  return(
    <div className="min-h-screen bg-[#f8fafc] flex">
      {/* Sidebar */}
      <div className="w- min-w- bg-white border-r p-5 flex flex-col h-screen sticky top-0">
        ...
        <div className="space-y-2 mb-4 max-h- overflow-auto">
          ...
          {boards.length===0 && <p className="text- text-gray-400">No boards yet</p>}
        </div>
        <div className="flex gap-2 mb-8">
          <input value={newBoardName} onChange={e=>setNewBoardName(e.target.value)} placeholder="New board name" className="border p-2 rounded-lg text-sm flex-1 min-w-0"/>
        ...
        <div className="mt-auto border-t pt-5">
          ...
          <p className="text- text-gray-500 mb-2">Board: <b className="text-black">{currentBoardName || "Select board"}</b></p>
          ...
          <button disabled={!selectedBoard} onClick={inviteUser} className="bg-blue-600 disabled:bg-gray-300 text-white w-full p-2.5 rounded-lg text-sm">Invite to Board</button>
          <p className="text- text-gray-400 mt-2">User must register first in this app.</p>
        </div>
        <button onClick={()=>{localStorage.clear(); setToken(null)}} className="mt-4 text-xs border p-2 rounded-lg hover:bg-gray-50">Logout</button>
      </div>
      ...
      <div className="flex-1 p-6 lg:p-8 overflow-auto">
        <div className="flex justify-between items-center mb-6 flex-wrap gap-3">
          ...
          <div className="flex gap-2">
            ...
            <select value={filterPrio} onChange={e=>setFilterPrio(e.target.value)} className="border px-3 py-2.5 rounded-lg bg-white text-sm"><option value="all">All Priority</option><option value="high">High</option><option value="medium">Medium</option></select>
          </div>
        </div>
          ...
          <div className="bg-white rounded-xl p-6 w- max-w-full max-h- overflow-y-auto">
            ...
            <div className="mt-6 border-t pt-4">
              ...
              <div className="max-h- overflow-y-auto mb-3 space-y-2 border rounded p-2 bg-gray-50">
                {taskComments.length===0? <p className="text-xs text-gray-400">No comments yet. Start discussion!</p> : taskComments.map(c => (
