...
app = FastAPI(title="WorkFlow SaaS API")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
def send_email_safe(to_email: str, subject: str, html_body: str):
    try:
        ...
        from_email = os.getenv("FROM_EMAIL", user or "noreply@workflow.com")
...
def get_user_boards(user, db):
    ...
    member_ids = [m.board_id for m in db.query(models.BoardMember).filter(models.BoardMember.user_id == user.id).all()]
    member_boards = db.query(models.Board).filter(models.Board.id.in_(member_ids)).all() if member_ids else []
    merged = {b.id: b for b in owned + member_boards}
...
def create_notification_safe(user_id, board_id, task_id, message, n_type="info", email_subject: Optional[str]=None):
    try:
        ...
        if user_email and email_subject:
            send_email_safe(user_email, email_subject, f"<h3>{email_subject}</h3><p>{message}</p>")
    except Exception as e:
        print("notif err", e)

@app.get("/")
def root():
    return {"ok": True, "email": bool(os.getenv("SMTP_HOST")), "cloudinary": CLOUDINARY_ENABLED}
...
def invite(board_id: int, payload: InviteRequest, current_user=Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if not exists:
        ...
        send_email_safe(payload.email, f"Invited to {board.name}", f"<p><b>{current_user.name}</b> invited you to <b>{board.name}</b></p>")
    return {"ok": True}
