...
def fix_db():
    try:
        with engine.connect() as conn:
            ...
            # COMMENTS TABLE FIX - ithaanu 500 fix
    ...
    try:
        ...
        for u in db.query(models.User).all():
            if not db.query(models.Board).filter(models.Board.owner_id==u.id).first():
                b=models.Board(name="My Workspace", owner_id=u.id); db.add(b); db.commit(); db.refresh(b)
                db.query(models.Task).filter(models.Task.user_id==u.id).filter(models.Task.board_id==None).update({models.Task.board_id:b.id}); db.commit()
        db.close()
    except Exception as e: print("fix boards err", e); traceback.print_exc()
...
def get_comments(task_id:int, current_user=Depends(get_current_user), db:Session=Depends(get_db)):
    try: return db.query(models.Comment).filter(models.Comment.task_id==task_id).order_by(models.Comment.id.asc()).all()
    except Exception as e: print("get comments", e); return []
