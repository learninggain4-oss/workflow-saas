...
import traceback, os, base64, re, io, csv
...
def get_user_profile(current_user = Depends(get_current_user)):
    return {"id": current_user.id, "email": current_user.email, "name": current_user.name, "subscription_tier": current_user.subscription_tier}
...
async def upload_file(file: UploadFile = File(...), current_user = Depends(get_current_user)):
    ...
    return {"url": f"data:{file.content_type};base64,{base64.b64encode(contents).decode('utf-8')}"}
...
def create_board(payload: schemas.BoardCreate, current_user = Depends(get_current_user), db: Session = Depends(get_db)):
    if current_user.subscription_tier == "free":
        if db.query(models.Board).filter(models.Board.owner_id == current_user.id).count() >= 3:
...
def rename_board(board_id: int, payload: schemas.BoardCreate, current_user = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if not b: raise HTTPException(status_code=403, detail="Not owner")
...
def invite(board_id: int, payload: schemas.InviteRequest, current_user = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if target:
        ...
        if not bm:
            ...
            bm.role = payload.role; db.commit()
    else:
        subject, html_body = f"Join {board.name}", f"<p>{current_user.name} invited you to {board.name} as {payload.role}. Please register.</p>"
...
def get_board_members(board_id: int, current_user = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if board:
        ...
        if owner: members.append({"email": owner.email, "name": owner.name, "role": "admin", "id": owner.id})
        for m in db.query(models.BoardMember).filter(models.BoardMember.board_id == board_id).all():
            ...
            if u: members.append({"email": u.email, "name": u.name, "role": m.role, "id": u.id})
    return members
...
async def create_task(payload: schemas.TaskCreate, current_user = Depends(get_current_user), db: Session = Depends(get_db)):
    if payload.board_id:
        ...
        if board_owner:
            ...
            if owner_u and owner_u.subscription_tier == "free" and db.query(models.Task).filter(models.Task.board_id == payload.board_id).count() >= 20:
